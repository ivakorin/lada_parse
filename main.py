...
bot = Bot(token='token') #TODO add yours bot token
dp = Dispatcher(bot)
...
async def checknews():
    c = LadaOnline()
    title = c.headers()
    last_news = open("last_news.txt", "r")
    ...
    if title == last_wrote_title:
        ...
        await bot.send_photo('chat_id', types.InputFile.from_url(c.image()), msg, parse_mode="MARKDOWN",    # TODO Put chat id here
                             reply_markup=inline_kb1)
        last_news = open("last_news.txt", "w")
